...
namespace GestureRecognizer {
	...
	public class GesturePatternEditor : Editor{
		...
		public override void OnInspectorGUI () {
			...
			GUILayout.BeginHorizontal (GUILayout.Width(w));
            if(currentGestureIndex < gestures.arraySize)
            {
                var closedLineProp = gestures.GetArrayElementAtIndex(currentGestureIndex);

                if (closedLineProp != null)
                {
                    closedLineProp = closedLineProp.FindPropertyRelative("closedLine");
